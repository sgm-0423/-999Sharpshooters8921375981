...
Waypoint targets[9] = {
    ...
    { 2.143109, 0.992947, 0.339523 },   // 靶4  实测 2026-08-19
    { 2.412625, 1.073127, -1.371876 },   // 靶5  实测 2026-08-19（更新：2.353828→2.412625, 0.954458→1.073127, -1.324164→-1.371876）
    { 1.710640, 0.874454, -1.850096 },   // 靶6  实测 2026-08-19
    { 1.685180, 1.497509, 1.954459 },   // 靶7  实测 2026-08-19,原点位xy：1.524588, 1.597363
    { 1.685180, 1.497509, -0.259734 },   // 靶8  实测 2026-08-19
    { 2.385051, 1.571942, 1.307579 },   // 靶9  实测 2026-08-19（更新：2.191065→2.385051, 1.380556→1.571942, 1.197381→1.307579）
};
...
// --- 开环速度控制（用于回程前进 8 秒 & 靶5 打完后的 90° 旋转）---
...
// 改动 2026-08-19：各靶打完后按需转 90°/180° 对准下一靶路线
...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < 9; i++)
    {
        // 改动 2026-08-20：暂时注释掉靶5节点（跳过靶5：不导航、不打靶、不旋转）
        // 说明：保留 targets[] 数组不变、用 continue 跳过 i==4（靶5），
        //       以免删掉数组元素导致后续靶位下标全部错位。
        if (i == 4)
        {
            ROS_INFO("Target 5 skipped (temporarily commented out)");
            continue;
        }
        ...
        // 改动 2026-08-20：靶4 到达后 → 顺时针转 90° → 停 1s → 再打靶
        if (i == 3)
        {
            ROS_INFO("Target 4 arrived, rotate clockwise 90 deg before shooting");
        ...
        // 改动 2026-08-19：靶3 打完 → 逆时针原地旋转 90° → 暂停 1s → 再前往下一节点
        if (i == 2)
        {
            ROS_INFO("Target 3 finished, rotate counter-clockwise 90 deg");
            ros::Duration(2.0).sleep();   // 改动 2026-08-20：旋转动作前停顿 2s
            rotate(cmd_vel_pub, 1.0, 90);
            ros::Duration(1.0).sleep();
        }

        // 改动 2026-08-19：靶4 打完 → 顺时针原地旋转 90° → 暂停 1s → 再前往下一节点
        if (i == 3)
        {
            ROS_INFO("Target 4 finished, rotate clockwise 90 deg");
            ros::Duration(2.0).sleep();   // 改动 2026-08-20：旋转动作前停顿 2s
            rotate(cmd_vel_pub, -1.0, 90);
            ros::Duration(1.0).sleep();
        }

        // 改动 2026-08-20：靶5 已跳过（见循环顶部 continue），此旋转块成为死代码，一并注释掉
        // if (i == 4)
        // {
        //     ROS_INFO("Target 5 finished, rotate clockwise 90 deg");
        //     rotate(cmd_vel_pub, -1.0, 90);
        //     ros::Duration(1.0).sleep();
        // }

        // 改动 2026-08-19：靶6 打完 → 逆时针原地旋转 180° → 暂停 1s → 再前往下一节点
        if (i == 5)
        {
            ROS_INFO("Target 6 finished, rotate counter-clockwise 180 deg");
            ros::Duration(2.0).sleep();   // 改动 2026-08-20：旋转动作前停顿 2s
            rotate(cmd_vel_pub, 1.0, 180);
            ros::Duration(1.0).sleep();
        }
