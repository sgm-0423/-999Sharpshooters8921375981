...
Waypoint targets[9] = {
    ...
    { 1.740331, 1.722084, 1.824515 },   // 靶7  [修改] 2026-08-20 tf_echo 重测（原 1.628311, 1.610672, 1.840913）
...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < 9; i++)
    {
        ...
        if (i == 8)
        {
            ROS_INFO("Target 9 finished, rotate counter-clockwise 120 deg");
            ros::Duration(2.0).sleep();   // 改动 2026-08-20：旋转动作前停顿 2s
            rotate(cmd_vel_pub, 1.0, 120);
            ros::Duration(1.0).sleep();
        }
    ...
    // ==== 改动 2026-08-20：回程改为"导航到回程节点 → 停 1s → 闭环导航到终点 → 前进 5s" ====
    // 原逻辑：到回程节点后开环前进 8 秒；现改为停 1s → 闭环导航到终点 → 前进 5s。
    ROS_INFO("===== Back to home node =====");
   // Move2goal(ac, 1.236615, 1.210887, -2.278829);

    //ros::Duration(1.0).sleep();   // [修改] 2026-08-20：到达回程节点后停 1s

    // [修改] 2026-08-20：停 1s 后闭环导航到终点前点（tf_echo 实测）
    Move2goal(ac, 0.286222, 0.766131, -2.137307);

    // [修改] 2026-08-20：到达终点前点后前进 4s到达终点
